...
namespace Dragonfly.Engine.Test.GraphicTests
{
    public class FullScreenTest : GraphicsTest
    {
        ...
        public override void CreateScene()
        {
            ...
            // create camera
            camera = new CompCamPerspective(CompTransformStack.FromLookAt(root, new Float3(0, 0, 5.0f), Float3.Zero));
            mainPass.Camera = camera;

            // create rotating node
            CompTransformStack rotation = new CompTransformStack(root);
            rotation.PushRotationY(new CompTimeSeconds(rotation), 0.5f);
