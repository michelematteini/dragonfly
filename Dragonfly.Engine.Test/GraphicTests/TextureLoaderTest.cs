...
namespace Dragonfly.Engine.Test.GraphicTests
{
    public class TextureLoaderTest : GraphicsTest
    {
        ...
        public override void CreateScene()
        {
            ...
            // create camera
            mainPass.Camera = new CompCamPerspective(CompTransformStack.FromLookAt(Context.Scene.Root, new Float3(0, 5.0f, 20.0f), Float3.Zero));

            // create rotating node
            CompTransformStack rotation = new CompTransformStack(Context.Scene.Root);
            rotation.PushRotationY(new CompTimeSeconds(rotation), 0.3f);
