...
namespace Dragonfly.Engine.Test.GraphicTests
{
    public class ShadowmapTest : GraphicsTest
    {
        ...
        void CreateOmniRoom(CompMtlPhong.Factory phongFactory, CompMtlPhong wallMaterial, Component root)
        {
            ...
            CompTransformStack lightsRotation = new CompTransformStack(root);
            CompTimeSeconds lrTime = new CompTimeSeconds(lightsRotation);
            lightsRotation.PushTranslation(new CompFunction<Float3>(lightsRotation, () => 0.5f * Float3.UnitX * (float)System.Math.Sin(lrTime.GetValue() * 1.3f)));
            lightsRotation.PushRotationY(lrTime, 0.8f);
            lightsRotation.PushTranslation(new CompFunction<Float3>(lightsRotation, () => 1.4f * Float3.UnitY * (float)System.Math.Sin(lrTime.GetValue())));

            // add point light
            Float3 lightPos = new Float3(1.4f, 1.5f, 0);
            CompLightPoint pointLight = new CompLightPoint(CompTransformStack.FromPosition(lightsRotation, lightPos), new Float3("#ff4001"), 40.0f);
        ...
        void CreateRadiusTestRoom(CompMtlPhong.Factory phongFactory, CompMtlPhong wallMaterial, Component root)
        {
            ...
            // add point light
            CompLightPoint pointLight = new CompLightPoint(CompTransformStack.FromPosition(root, new Float3(1.4f, 0.5f, 0)), new Float3("#eeeeff"), 20.0f);
            pointLight.CastShadow = true;
        ...
        void CreateForestSpotRoom(CompMtlPhong.Factory phongFactory, CompMtlPhong wallMaterial, Component root)
        {
            ...
            CompLightSpot spotLight1 = new CompLightSpot(CompTransformStack.FromPosAndDir(root, spotPos1, -spotPos1), new Float3("#c0ff00"), 40.0f, (20.0f).ToRadians(), (80.0f).ToRadians());
            spotLight1.CastShadow = true;
            spotLight1.Radius.Set(0.1f);
            spotLight1.AddDebugMesh();

            Float3 spotPos2 = new Float3(-1.8f, 1.0f, -3.0f);
            Float3 spotDir2 = (new Float3(0, 2.0f, 0) - spotPos2).Normal();
            CompLightSpot spotLight2 = new CompLightSpot(CompTransformStack.FromPosAndDir(root, spotPos2, spotDir2), new Float3("#00e7ff"), 40.0f, (10.0f).ToRadians(), (80.0f).ToRadians());
        ...
        void CreateCartMultispotRoom(CompMtlPhong.Factory phongFactory, CompMtlPhong wallMaterial, Component root)
        {
            ...
            for (int i = 0; i < spotsCount; i++)
            {
                ...
                CompLightSpot spotLight = new CompLightSpot(CompTransformStack.FromPosAndDir(lightsRotation, lPos, - lPos + Float3.UnitY), new Float3("#ff005e"), 20.0f, (5.0f).ToRadians(), (70.0f).ToRadians());
        ...
        public override void CreateScene()
        {
            ...
            // create objects
            {
                ...
                for (int x = -halfGrid; x < halfGrid; x++)
                {
                    for (int y = -halfGrid; y < halfGrid; y++)
                    {
                        ...
                        CompTransformStack roomNode = new CompTransformStack(root);
                        roomNode.Push(Float4x4.Translation(roomPos));
                        CreateRoom(phongFactory, whiteMat, roomNode, (x + y + 2 * halfGrid) % 4);
                    }
                }
            }
            ...
            CompLightDirectional sunLight = new CompLightDirectional(CompTransformStack.FromDirection(root, new Float3(1.0f, -5.0f, -1.0f)), new Float3("#c2e1eb"), 1.0f);
            sunLight.CastShadow = true;

            // display the shadowmap atlas
            CompRenderBuffer shadowAtlas = typeof(BaseMod).GetProperty("ShadowAtlas", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).GetValue(baseMod) as CompRenderBuffer;
